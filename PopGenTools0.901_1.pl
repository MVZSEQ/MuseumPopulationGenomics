...
##version 0.900 compatable with angsd0.900
...
spacemix: generating spacemix input files using ANGSD genotype (-doGeno 2)
 ...
 # open($file, $raw_ref); 
  ...
  while (<IN>) {
    ...
    if ($line =~ m/^>(\S+)/) {
      ...
      $sequence_data{$ids} .= $line;
    }
  }
  close IN;

  foreach my $contig (sort {$a cmp $b} keys %sequence_data) {
    print OUT ">", $contig, "\n";
    print OUT $sequence_data{$contig}, "\n";
  }
  close OUT;
  ...
  sub mask{
    ...
    foreach (keys(%data)) {
      ...
      $newdata {substr($_,10)} = $data{$_};
    } 
    for my $key (sort {$a<=>$b} keys %newdata) {
      print OUT '>scaffold_'.$key, "\n", $newdata{$key},"\n";
    } 
    close OUT;
    unlink ($new_raw_ref);
  }
  ...
  sub final {
	...
	if ((length ($fix{$line})) < (length ($ori{$line}))) {
	  ...
	  #print $newseq, "\n";
	  $newfix {substr($line, 10)} = $newseq; 
	}
	elsif ((length ($fix{$line})) > (length ($ori{$line}))) {
	}
	else {
	  $newfix {substr($line, 10)} = $fix{$line};	  	  
	}
      }
      else  {
	$newfix{substr($line, 10)} = 'N' x length($ori{$line});
      }      
    }
...
Options: -g FILE raw genotype probability generated using ANGSD "-doGeno 4" (e.g. Contig29 294 TT TT TA....)
         ...
         -m FLOAT missing data allowed [0.3]
         -i       if i is supploed, only choose informative sites
         -p       if p is supplied, Random choose one SNP per contig.
\n/) if (!@ARGV);
  
  my %opts = (g=>undef, n=>undef, s=>undef,o=>undef, h=>undef, m=>0.3);
  getopts('n:g:s:o:m:h:pi', \%opts);
  ...
  unless ($opts{p}) {
	...
	$tt++ if $m eq 'TT';
      ...
      push  @all, $aa unless $aa == 0; 
      push  @all, $gg unless $gg == 0;
      push  @all, $cc unless $cc == 0;
      push  @all, $tt unless $tt == 0;
	...
	if ($hets > 1 ) {
	  if ($site == $size && $missing/$site < $miss) {	    
	    $hash{$line[0]}{$line[1]} = join ("\t",@line[4..$#line] );
	     print OUT $line[0], "\t", $line[1], "\t", join("\t",$hash{$line[0]}{$line[1]}), "\n";
	  }
	}      
	if ($hets == 1) {
	  if ( scalar @all == 2 && $site == $size && $missing/$site < $miss) {
	    $hash{$line[0]}{$line[1]} = join ("\t",@line[4..$#line] );
	    print OUT $line[0], "\t", $line[1], "\t", join("\t",$hash{$line[0]}{$line[1]}), "\n";
	  }
	}
       	...
       	if ($hets == 0 ) {
	  if ( min (@all) > 1 ||  $site == $size && $missing/$site < $miss) {
	    $hash{$line[0]}{$line[1]} = join ("\t",@line[4..$#line] );
	     print OUT $line[0], "\t", $line[1], "\t", join("\t",$hash{$line[0]}{$line[1]}), "\n";
	  }
	}
    ...
    my $start4 = time;
    my $out1 = 'cleaned';
    trim($opts{n},$n,$out,$out1,\%genos);
    my $time4 = int(time - $start4);
    print "Xixi...Trimmed original genotype matrix in $time0 sec!\n";
    ...
    my $start1 = time;
    my $out2= 'converted';
    transposex($out1, $opts{n}, $n, $out2); #transpose the matrix;
    my $time1 = int(time - $start1);
  ...
  if ($opts{p}) {
	...
	if ($hets == 1) {
	  ...
	  if ( scalar (@all) == 2  && min (@all) > 1  &&  $site == $size && $missing/$site < $miss) {
    ...
    my $start1 = time;
    my $out2= 'converted';
    transposex($out, $opts{n}, $n, $out2); #transpose the matrix;
    my $time1 = int(time - $start1);
    print "Hehe...Transposed cleaned genotype matrix in $time1 sec! \n";
...
Options: -g     FILE    raw genotype probability generated using ANGSD "-doGeno 2" (e.g. Contig29 294 A T 0 1 2 -1 -1 2 1....)
         -n     INT     sample size
         -s     INT     number of loci
         -o     FILE    outfile name
         -h     INT     1 = all sites; 0 = one snp per contig 
         -m     FLOAT   missing data cutoff? [0.2]

\n/) if (!@ARGV);
         
  
  my %opts = (g=>undef, n=>undef, s=>undef,o=>undef, h=>1, m=>0.2);
  getopts('n:g:s:o:h:m:p', \%opts);
 ...
 unless ($opts{p}) {
    ...
    transposex('trimmed', $opts{n}, $add, 'transposed'); #transpose the matrix;
	...
	if ($m eq '2' || $m eq '1') {
	 ...
	 $site++; $missing++;
       ...
       if ($add > 1 && $site == $size && ($missing/$site <= $opts{m})) {
	 $hash{$line[0]}{$line[1]} = join ("\t",@line[2..$#line] ); 	
    ...
    system("rm selected");
    transposex('trimmed', $opts{n}, $n, 'transposed'); #transpose the matrix;
  ...
  sub jasonBragg {
   ...
   foreach (sort {$a cmp $b} keys %hash) { 
    ...
    transposex('trimmed', $size, $j, 'transposed'); #transpose the matrix;
...
Basic:
         ...
         -A INT   only use concordant PE mapping? 1=yes, 0=no. [1]
         -t INT   number of threads [4]
         -g INT   genotype output options [3]
...
Theta Estimate:
         -T       estimating per-site thetas? [null] when use -T, no SNP\/Genotype calling should be performed.

Posterior prob of pre-site allele frequencies? [null]

         -p       Compute posterior prob of pre-site allele frequencies
...
\n/) if (!@ARGV);
 ...
 my %opts = (b=>undef, f=>undef, r=>undef,a=>undef, F=>1, Q=>20, A=>1, q=>2, c=>0, g=>3, o=>undef,t=>4,n=>undef,c=>0,s=>0,P=>0.95, e=>1, h=>0, d=>0, v=>1e-6,m=>0);
 getopts('b:f:r:a:F:Q:A:q:n:c:g:o:t:c:s:P:e:h:d:v:m:Tp', \%opts);
 my $angsd = 'angsd';
 my $emOptim = 'realSFS';
 ...
 if ($opts{F} == 1 ) { #folded
   if ($opts{c} == 1) { #call genotypes
     if ($opts{s} == 1) { #call SNPs
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1 -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d} -SNP_pval $opts{v} -minMaf $opts{m}   -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       system (" $emOptim $opts{o}.saf.idx  -P $opts{t} >  $opts{o}.saf.sfs") if ($opts{T} || $opts{p}) ;
       system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1   -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d} -SNP_pval $opts{v} -minMaf $opts{m}   -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs  ") if $opts{T};
       system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1   -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d} -SNP_pval $opts{v} -minMaf $opts{m}   -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.postprob   -pest  $opts{o}.saf.sfs  ") if $opts{p};
          
     }
     if ($opts{s} == 0) {       
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A} -minMapQ $opts{q} -minQ $opts{Q} -fold 1 -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d}  -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       system (" $emOptim $opts{o}.saf.idx -P $opts{t} >  $opts{o}.saf.sfs") if ($opts{T} || $opts{p});
       
       system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1  -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d}  -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs  ") if $opts{T};
        system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1  -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d}  -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.postprob  -pest  $opts{o}.saf.sfs  ") if $opts{p};
   ...
   if ($opts{c} == 0) { 
     ...
     if ($opts{s} == 1) { #call SNPs
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A} -minMapQ $opts{q} -minQ $opts{Q} -fold 1 -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e} -SNP_pval $opts{v} -minMaf $opts{m} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       system (" $emOptim $opts{o}.saf.idx -P $opts{t} >  $opts{o}.saf.sfs") if ($opts{T} || $opts{p}) ; 
       
       system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1  -GL 1  -doGeno $opts{g}  -doPost $opts{e} -SNP_pval $opts{v} -minMaf $opts{m} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf   -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs ") if $opts{T};

        system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1  -GL 1  -doGeno $opts{g}  -doPost $opts{e} -SNP_pval $opts{v} -minMaf $opts{m} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf   -out $opts{o}.postprob   -pest  $opts{o}.saf.sfs ") if $opts{p};

         
     }
     if ($opts{s} == 0) {       
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1 -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       system (" $emOptim $opts{o}.saf.idx  -P $opts{t} >  $opts{o}.saf.sfs") if  ($opts{T} || $opts{p}) ;      
       system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1   -GL 1  -doGeno $opts{g}  -doPost $opts{e} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs ") if $opts{T};

        system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q} -fold 1   -GL 1  -doGeno $opts{g}  -doPost $opts{e} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf -out $opts{o}.postprob   $opts{o}.saf.sfs ") if $opts{p};
  ...
  if ($opts{F} == 0 ) { #unfolded
   if ($opts{c} == 1) { #call genotypes
     if ($opts{s} == 1) { #call SNPs
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a} -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q}  -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d} -SNP_pval $opts{v} -minMaf $opts{m}   -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       my $unfolded = 2*$opts{n};
       system (" $emOptim $opts{o}.saf.idx  -P $opts{t} >  $opts{o}.saf.sfs") if  ($opts{T} || $opts{p}) ;      
       
       system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a} -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q}   -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d} -SNP_pval $opts{v} -minMaf $opts{m}   -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf    -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs ") if $opts{T} ;

       system (" $angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a} -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q}   -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d} -SNP_pval $opts{v} -minMaf $opts{m}   -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf    -out $opts{o}.postprob  -pest  $opts{o}.saf.sfs ") if $opts{p} ;
         
     }
     if ($opts{s} == 0) {       
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q}  -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d}  -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       my $unfolded = 2*$opts{n};
       system (" $emOptim $opts{o}.saf.idx -P $opts{t} >  $opts{o}.saf.sfs") if ($opts{T} || $opts{p}) ;         
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A} -minMapQ $opts{q} -minQ $opts{Q}   -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d}  -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf    -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs ") if $opts{T};

       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A} -minMapQ $opts{q} -minQ $opts{Q}   -GL 1  -doGeno $opts{g}  -doPost $opts{e}  -postCutoff $opts{P}   -doCounts 1 -doGlf 2 -geno_minDepth $opts{h} -minInd $opts{d}  -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf    -out $opts{o}.postprob  -pest  $opts{o}.saf.sfs ") if $opts{p};
   ...
   if ($opts{c} == 0) { 
     ...
     if ($opts{s} == 1) { #call SNPs
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A} -minMapQ $opts{q} -minQ $opts{Q}  -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e} -SNP_pval $opts{v} -minMaf $opts{m} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       my $unfolded = 2*$opts{n};
       system (" $emOptim $opts{o}.saf.idx -P $opts{t} >  $opts{o}.saf.sfs") if ($opts{T} || $opts{p}) ; 
       
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A} -minMapQ $opts{q} -minQ $opts{Q}    -GL 1  -doGeno $opts{g}  -doPost $opts{e} -SNP_pval $opts{v} -minMaf $opts{m} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs ") if $opts{T} ;

       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A} -minMapQ $opts{q} -minQ $opts{Q}    -GL 1  -doGeno $opts{g}  -doPost $opts{e} -SNP_pval $opts{v} -minMaf $opts{m} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.postprob  -pest  $opts{o}.saf.sfs ") if $opts{p} ;

       
         
     }
     if ($opts{s} == 0) {       
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a} -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q}  -out $opts{o}  -GL 1  -doGeno $opts{g}  -doPost $opts{e} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf");
       my $unfolded = 2*$opts{n};
       system (" $emOptim $opts{o}.saf.idx -P $opts{t} >  $opts{o}.saf.sfs") if ($opts{T} || $opts{p}) ;        
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q}   -GL 1  -doGeno $opts{g}  -doPost $opts{e} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.thetas  -doThetas 1 -pest  $opts{o}.saf.sfs") if $opts{T};
       system ("$angsd -bam $opts{b} -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $opts{A}  -minMapQ $opts{q} -minQ $opts{Q}   -GL 1  -doGeno $opts{g}  -doPost $opts{e} -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf  -out $opts{o}.postprob  -pest  $opts{o}.saf.sfs") if $opts{p};
...
\n/) if (!@ARGV);
  ...
  my %opts = (b=>undef, f=>undef, r=>undef, a=>undef, A=>1, s=>undef, Q=>20, q=>2, G=>1, l=>100000, o=>undef, t=>4,g=>3);
  getopts('b:f:r:a:s:Q:q:G:A:l:o:t:g:', \%opts);
  ...
  if ($opts{o} =~ m/\/$/ ){
	...
	system ("$angsd -bam $dir$prefix.txt -sites $opts{f} -ref $opts{r} -anc $opts{a}  -only_proper_pairs $mapping  -minMapQ $opts{q} -minQ $opts{Q} -out $prefix  -GL $opts{G}  -doGeno $gcall  -doPost 1 -doMaf 2 -doMajorMinor 1 -doSaf 1 -rf $rf -nThreads $threads");
  ...
  my @probs = <*.saf.idx>;
	...
	if ($prefix1 ne $prefix2) {
	  ...
	  #system ("gunzip $prefix1.saf.gz  $prefix2.saf.gz ");
...
\n/) if (!@ARGV);
  ...
  unless ($opts{p}) { 
    ...
    transposex($out, $opts{n}, $add, $out2);
  }
  ...
  if ($opts{p}) {
    ...
    my $out2 = $opts{o} . "_SPA.geno";
    transposex($before, $opts{n}, $n , $out2);
...
\n/) if (!@ARGV);
  ...
  sub treemix {
	...
	if ($_ !~ m/^$/) {
	  ...
	  $ind{$c} = {'ref' => $ref,'alt' => $alt, 'pop'=> $pop{$c}};   
	  $c++;
	}
      }
	...
	foreach my $i (sort {$a <=> $b} keys (%ind)) {	
	    if ($ind{$i}{'pop'} ==  $ID ) {	  	  
	      $pop_all{$ID}{'ref'} += $ind{$i}{'ref'};
	      $pop_all{$ID}{'alt'} += $ind{$i}{'alt'};
	    } 
	  }
      }
	 
      foreach my $popid (sort {$a <=> $b} keys (%pop_all)) {
	    print OUT  $pop_all{$popid}{'ref'}, "," , $pop_all{$popid}{'alt'}, "\t";
      }
      print OUT "\n";
...
\n/) if (!@ARGV);
 ...
 close GEO; close GOUT;
  ...
  unless ($opts{p}) {
    ...
    my $start1 = time;
    my $out2= 'converted2';
    transposex($out, $opts{n}, $opts{s}, $out2); #transpose the matrix;
    my $time1 = int(time - $start1);
    print "Hehe...Transposed cleaned genotype matrix in $time1 sec! \n";
...
\n\n/) unless (@ARGV);
  ...
  sub nexus {
    ...
    #system ("cp transposed transposed1");
    open (IN, "<", 'transposed');
...
\n\n/) unless (@ARGV);
  ...
  my $out2= 'converted';
  transposex($out1, $sample , $n, $out2); #transpose the matrix;
...
\n\n/) unless (@ARGV);
  ...
  system("rm selected");
  transposex('trimmed', $sample, $n, 'transposed'); #transpose the matrix;
...
\n\n/) unless (@ARGV);
  ...
  transposex($before, $sample, $n , $out2);
...
\n/) if (!@ARGV);
  ...
  while (<IN>) {
    ...
    if ($a == 1){
      if ($all/$samples <= $opts{s} ) {
	if ($geno0 + $genom < $samples && $geno2 + $genom < $samples && $geno1 + $genom < $samples) {
	  print OUT $_;
	}
	else {
	  print OUT2 $_;
...
\n\n/) unless (@ARGV);
  ...
  close IN2;
  my $out2 = "input.geno";
  transposex($newgeno, $sample, $add, $out2); 
...
Options: -f           folder containning all bams
         -r           reference genome
         -o           outdir
         -s           s th percentile to trim off loci with extremely low depth [2.5]
         -b           b th percentile to trim off loci with extremely high depth [97.5]
         -m           Percent missing data defined as no data [0.3]
         -n           Percent individuals that fail to pass -m filter 
                      defined as missing loci [0.3]
         -H           H th percentile to trim off loci with extremely 
                      high average heterozygosity [97.5]
         -d           minimal depth [3]
         -l           indel filter [5]
...
my %opts = (f=>undef, r=>undef,d=>3,o=>undef,a=>undef, l=>5,m=>0.3, n=>0.3, H=>97.5, s=>2.5,b=>97.5);
getopts('f:r:d:o:a:l:m:n:H:b:s:', \%opts);
...
foreach (@file) {
  ...
  system ("samtools mpileup -ugEf $ref $file | bcftools call -c -  | vcfutils.pl vcf2fq -d $depth1 -l 5  >  $fa ");
  my $fa2 = $fa."1";
  system ("seqtk seq $fa -A > $fa2");
  system ("mv $fa2 $fa");


  my $depth = $outdir . $lib . ".depth";
  system ("samtools depth $file > $depth"); 
  ...
  foreach my $g ( keys %gene) {
    ...
    $per_gene_depth{$g} = $gene{$g}{'dep'}/$gene{$g}{'count'};
  ...
  while (<FIX>) {
    ...
    if ($line  =~ m /^>(\S+)/ ) {
      ...
      $line =~ s/[a|c|g|t|n|y|s|k|r|w|m]/N/g ;
 ...
 foreach my $sample (sort {$a cmp $b} keys %newfix) {
   foreach my $contig (sort {$a cmp $b} keys %{$newfix{$sample}}) {
     my $length = length ($newfix{$sample}{$contig});
     my $seq1 = $newfix{$sample}{$contig};
     my $N = ($seq1 =~ s/N//ig) - $temf{$contig}{'N'};
     
     $seq1 = $newfix{$sample}{$contig};
     
     my $H = ($seq1 =~ s/[Y|W|R|S|K|M]//ig);
     
     $missing{$contig}{'N'}++  if ($N/$temf{$contig}{'ef'} >= $ma);

     
     $missing{$contig}{'H'} += $H/$temf{$contig}{'ef'};
     
   }    
 }

 my @COV;
 foreach my $gene (keys %per_gene_depth) {
  push @COV, $per_gene_depth{$gene}/$size;
 }
 ...
 my %delete;
 my $dd= 0;
 foreach my $contig (sort {$a cmp $b} keys %missing) {
   $missing{$contig}{'N'} = 0.000 if (!$missing{$contig}{'N'});
   $missing{$contig}{'H'} = 0.000 if (!$missing{$contig}{'H'});  
   $delete{$contig}++ if ($missing{$contig}{'N'}/$size >= $mi);
   $dd++ if ($missing{$contig}{'N'}/$size >= $mi);
   $missing{$contig}{'H'} = $missing{$contig}{'H'}/$size;
   
   push @H, $missing{$contig}{'H'};
   }

 print $dd, " loci were removed from the analyses becasue they are missing data!", "\n";
   
 my $h_percentile = $outdir . "Marker_averageH_percentile.txt";
 my $c_percentile = $outdir . "Marker_averageCov_percentile.txt";
 
 my ($lowH, $highH) = percentile (\@H, $h_percentile, '0.5', $mh);
 my ($lowper, $highper) = percentile (\@COV, $c_percentile, $min, $max);
 
 my $tt = 0;
 foreach my $contig (sort {$a cmp $b} keys %missing) {
   $delete{$contig}++ if ($missing{$contig}{'H'}>= $highH);
   
   $tt++ if  ($missing{$contig}{'H'} >= $highH);
 }
 print $tt, " loci were removed from the analyses becasue of high average heterozygosity!", "\n";
   
 my $cc;
 foreach my $gene (keys %per_gene_depth) {  
   $delete{$gene}++ if ($per_gene_depth{$gene}/$size <= $lowper || $per_gene_depth{$gene}/$size >= $highper);
   $cc ++  if ($per_gene_depth{$gene}/$size <= $lowper || $per_gene_depth{$gene}/$size >= $highper);
 }
 print $cc, " loci were removed from the analyses becasue of extremely low or high coverage!", "\n";
 
 foreach my $sample (sort {$a cmp $b} keys %newfix) {
   foreach my $contig (sort {$a cmp $b} keys %{$newfix{$sample}}) {
     unless ($delete{$contig}) {
       $newfix2{$sample}{$contig} = $newfix{$sample}{$contig};
 ...
 my $c = 0;
 my $locus =1;
 foreach my $sample (sort {$a cmp $b} keys %newfix2) {
   foreach my $contig (sort {$a cmp $b} keys %{$newfix2{$sample}}) {
     
     print PAR "DNA,", "p", $locus, " = ";
     print PAR $c+1, "-", $c+length ($newfix2{$sample}{$contig}), "\n";
     $locus++;
     $c += length ($newfix2{$sample}{$contig});
     $all += length ($newfix2{$sample}{$contig});
...
Options: 
-g     FILE    raw genotype probability generated using ANGSD 
               "-doGeno 2" (e.g. Contig29 294 A T 0 1 2 -1 -1 2 1....)
-o     CHAR    outfile prefix 

\n/) if (!@ARGV);
         
  
  my %opts = (g=>undef, o=>undef);
  getopts('g:o:', \%opts);


  
  my $geno = $opts{g} || die "need a genotype input file!\n";
  my $out = $opts{o} || die "what is the outfile prefix?\n";

  
  my $out2 = dirname ($geno) . "/" . $out. "_input.geno";
  my @array;
  my $d = 0; 
  open (my $genoin, "<", $geno);
  while (<$genoin>){
  ...
  close $genoin;
  my @out=transpose(\@array);
  my $n = scalar @{$out[0]};
  my $newgeno = dirname ($geno) . "/" . $out. '_allele_counts.txt';
  my $samplesize = dirname ($geno) . "/" . $out. '_sample_size.txt';
  
  open (IN, "<", $out2);
  open (ALLELE, ">", $newgeno);
  open (SAMPLE, ">", $samplesize);

  print ALLELE "Sample", "\t";
  print SAMPLE "Sample", "\t";
  foreach (1 .. $n) {
    my $d = $_;  
    print ALLELE "locus", $d, "\t";
    print SAMPLE "locus", $d, "\t";
  }
  print ALLELE "\n";
  print SAMPLE "\n";
